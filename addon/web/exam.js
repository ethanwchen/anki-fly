...
class Exam {
  ...
  frame(now) {
    ...
    if (!this.running) { const sp = this.sim.run(dt); void sp; }
  ...
  async start(payload, seed = 7) {
    ...
    $('sub').textContent = `${cards.length} cards · ${payload.search}`;
  ...
  report(res) {
    ...
    $('report').innerHTML = `
      ...
      <div class="foot">How this works: each card's note id is turned into an odor (6 glomeruli) and presented to the fly's mushroom body; the fly's preference is read from its plastic Kenyon-cell → MBON synapses, which your Good/Again answers have been rewiring. "What FSRS predicts" samples each card as correct with probability R (Anki's own retrievability: R = (1 + factor·t/S)^(−decay), evaluated now) — that row is the real gauge. "What the fly got" blends in the fly's toy brain at ${pct(FLY_WEIGHT)} (new cards ≈ 2%). Retention, efficiency and consistency come straight from your collection and review log using Anki's own definitions (true retention counts rated, scheduling-affecting reviews; mature = interval ≥ 21 d). Thresholds in the diagnosis are conventions, each with its research reason. Cards without FSRS memory state use an SM-2 heuristic (0.9^(elapsed/interval)) and are labelled as such.</div>`;
