// Same behavioural checks as test_circuits.mjs, run against the female (FlyWire FAFB v783b) pack.
import { Sim, parseGraph } from '../addon/web/sim.js';
import { readFileSync } from 'node:fs';
import assert from 'node:assert/strict';
const meta = JSON.parse(readFileSync(new URL('../addon/web/data/female/meta.json', import.meta.url)));
const G = meta.groups;
const load = () => { const sim = new Sim(parseGraph(readFileSync(new URL('../addon/web/data/female/graph.bin', import.meta.url)).buffer.slice(0)));
